#!/usr/bin/env python3
"""
DVT-UNet3D pretraining across multiple training stacks with validation-based
model selection.
...
────────────
  ...
  3. Trains DVT-UNet3D using its existing 2-stage N2V flow, but with multi-stack
  ...
  4. Every --checkpoint-interval iters:
       ...
       - If mean val stSNR is best so far, saves --output-dir/dvt_unet3d_best.pth.
       - Always overwrites --output-dir/dvt_unet3d_last.pth.
...
DEFAULTS
────────
By default, refuses to overwrite an existing dvt_unet3d_best.pth in
--output-dir (so you don't lose a good run by accident). Pass
--force-overwrite to bypass.

CONFIG REQUIREMENTS
───────────────────
The config file at --config (default: configs/dvt_unet3d_pretrain.py) must
define a CONFIG dict with at least: base_ch, token_dim, grid_shape,
n_vit_blocks, n_heads, patch_d, patch_hw, batch_size, lr, mask_ratio,
mask_radius, normalization, temporal_target. warmup_iters and n2v_iters
in the config are OVERRIDDEN by --total-iters split: 1/25 of total iters
go to warmup (or use --warmup-iters to override).
"""
...
# ══════════════════════════════════════════════════════════════
# 3D augmentation (same as in dvt_unet3d.py)
# ══════════════════════════════════════════════════════════════
...
def run_pretraining(args):
    print("=" * 70)
    print("DVT-UNet3D PRETRAINING")
    print("=" * 70)

    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    best_ckpt_path = output_dir / "dvt_unet3d_best.pth"
    last_ckpt_path = output_dir / "dvt_unet3d_last.pth"
    ...
    for k in ("base_ch", "token_dim", "grid_shape", "n_vit_blocks",
              "n_heads", "patch_d", "patch_hw", "batch_size", "lr",
...
def parse_args():
    ...
    p.add_argument("--config", type=Path,
                    default=ROOT / "configs" / "dvt_unet3d_pretrain.py",
                    help="DVT config file. Default: configs/dvt_unet3d_pretrain.py")
    ...
    p.add_argument("--force-overwrite", action="store_true",
                    help="Allow overwriting an existing "
                         "<output-dir>/dvt_unet3d_best.pth. By default, "
